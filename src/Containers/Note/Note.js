...
export class Note extends Component {
  ...
  render = () => {
    const filteredUnChecked = this.state.listItems.filter(
      note => !note.checked
    );
    ...
    return (
      <div
        draggable
        onDragStart={() => {
          this.props.startDrag(this.props.note.id);
        }}
        onDragOver={() => {
      ...
      >
        ...
        {unchecked}
        {checked.length > 0 && <div className="check-section">{checked}</div>}
      </div>
    );
  };
}
export const mapDispatchToProps = dispatch => ({
  ...
  changeNoteOrder: (noteOne, noteTwo) =>
    dispatch(changeNoteOrder(noteOne, noteTwo))
});
