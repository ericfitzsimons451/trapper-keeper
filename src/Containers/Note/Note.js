...
export class Note extends Component {
  ...
  displayFilteredUnChecked = () => {
    ...
    return filteredUnChecked.map((text, i)=> {
      return (
        <div key={i} id={text.id} className="text">
  ...
  render = () => {
    ...
    return (
      <div 
      draggable 
      onDragStart={() => { this.props.startDrag(this.props.note.id) }} 
      onDragOver={() => { this.onDragOver(this.props.note.id) }} 
      onDragEnd={() => { this.props.patchNotes(this.props.notes) }} 
      className="note" 
