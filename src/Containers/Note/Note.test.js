...
describe.skip[('Note', () => {
    ...
    beforeEach(() => {
        mockState = [{id: 4, text: 'yo', checked: false}, {id: 5, text: 'hello', checked: false}]
        mockHistory = { push: jest.fn() }
        mockNote = {
            title: 'hello',
            listItems: [{id: 4, text: 'yo', checked: false}, {id: 5, text: 'hello', checked: false}]
        }
    ...
    describe('openNote', () => {
        it.skip('should push a note id onto the history object if the className is NOT checkbox, uncheckbox, or delete-button', () => {
            const mockPushToHistory = jest.fn()
            const mockId = 9
            const mockEvent = {target: {parentElement: {className: 'update' } }}
            wrapper.instance().openNote(mockEvent)
            expect(mockPushToHistory).toHaveBeenCalledWith(mockId)
        })
